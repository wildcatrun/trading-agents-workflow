...
function usage() {
  ...
  trading-agents-workflow trade-intent --asset TYPE --symbol SYMBOL --side SIDE --proposal ID --risk ID --pre-order-risk-audit ID --human-gate ID --protocol-audit ID --freshness-checked-at ISO --workflow-id ID --trace-id ID --expires-at ISO --price-constraints JSON --risk-limits JSON [--intent-id ID] [--quantity N] [--order-type TYPE] [--actor flashcat] [--assurance mtls] [--cert FINGERPRINT] [--source codex_mtls] [--idempotency-key KEY] [--root DIR]
  trading-agents-workflow trading-core-receipt --intent ID --human-gate ID --freshness-checked-at ISO [--workflow-id ID] [--trace-id ID] [--status accepted|submitted|filled|rejected] [--ref REF] [--receipt-id ID] [--summary TEXT] [--root DIR]
    ...
    case "workflow-control-loop-tick":
      return {
        ...
        input: {
          ...
          workflowSuperviseCooldownMs: options["workflow-supervise-cooldown-ms"],
          idleWorkflowSuperviseCooldownMs: options["idle-workflow-supervise-cooldown-ms"],
