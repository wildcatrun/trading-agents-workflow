...
import { canonicalWorkflowAction } from "./workflow/action-aliases.js";
export { canonicalWorkflowAction } from "./workflow/action-aliases.js";
...
async function recordWorkflowActionMigrationTelemetry(rootDir, action, requestedAction, input = {}, permissionDecision = null) {
  const migration = workflowActionMigrationInfo(action);
  if (!migration) return null;
  const permissionRule = WORKFLOW_ACTION_PERMISSION_RULES[action];
  if (permissionRule?.mutating !== true && migration.migrationStatus !== "deprecated") return migration;
  try {
    const paths = await ensureWorkflowLayout(rootDir, input);
    const caller = permissionDecision?.caller || workflowPermissionCaller(input);
    const createdAt = nowIso();
    await appendWorkflowEvent(paths, {
      eventType: "workflow.action_migration_telemetry",
      status: migration.migrationStatus || "legacy_active",
      workflowId: input.workflowId || input.workflow_id || "",
      traceId: input.traceId || input.trace_id || "",
      actor: caller.agentId || input.actor || input.from || "unknown",
      sourceRuntime: caller.runtime || input.runtime || "",
      sourceAgent: caller.agentId || input.agentId || input.agent_id || "",
      nextState: migration.decisionClass || "legacy_active",
      payload: {
        action,
        requestedAction,
        ...
        legacyCompatibilitySource: input.legacyCompatibilitySource || input.legacy_compatibility_source || "",
        permissionAllowed: permissionDecision?.allowed ?? null,
        telemetryOnly: true
      },
      createdAt
    });
...
INSERT INTO workflow_v2_human_gate_packages (
  package_id,
  workflow_id,
  plan_id,
  source_review_id,
...
} = WORKFLOW_V2_INTERVENTION_ACTION_HANDLERS;
