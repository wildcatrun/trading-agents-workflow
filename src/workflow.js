...
export const SCHEDULE_ACTION_HANDLERS = createScheduleActionHandlers({
  boolOption,
  ensureWorkflowLayout,
  firstText,
  nextScheduleRunAt,
  ...
  parseJsonValue,
  scheduleRow,
  WORKFLOW_SCHEDULE_CONCURRENCY_POLICIES,
  WORKFLOW_SCHEDULE_MISFIRE_POLICIES
});
...
export async function runWorkflowAction(rootDir, input = {}) {
  ...
  const incidentResult = await runIncidentAction(INCIDENT_ACTION_REGISTRY, action, rootDir, input);
