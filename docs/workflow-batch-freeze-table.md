...
| `route_shell.ingest`, `route-shell.ingest`, `route_shell.route` | F | `source_frozen` | Batch E proved live state was clean; Batch F closes source entrypoints so route-shell cannot be reactivated by CLI/action contract/Gateway hook/aliases/permission. Batch F notes: `docs/workflow-batch-f-route-shell-source-freeze.md`. | `message_flow.send` and direct runtime adapter dispatch from `runtime_agents`; no v2 migration. | `must_not_depend`; v2 must not depend on route shell and must keep backend rejection. | Historical tests and `src/route-shell-actions.js` implementation only; `meeting.dispatch runtime=openclaw_route_shell` and `runtime.bridge.drain runtime=openclaw_route_shell` now fail closed. | Keep source-frozen; next deletion batch may remove `src/route-shell-actions.js` and normalization remnants after full regression. | Any public CLI/MCP/plugin action, alias, permission, migration metadata, Gateway hook, successful route-shell redirect, or v2 dependency returns. | Revert Batch F commit if a real caller is found; otherwise restore specific code from Git history only as an archive diagnostic. |
...
- Batch F is source-frozen for route-shell and should progress to deletion only after a full regression confirms no useful dependency.
