...
| `workflow.advance`, `workflow.supervise` | C | `legacy_active` | Mutating behavior still owns legacy dispatch sync, task transitions, checkpoint, runtime drain, and closeout gaps; Batch C audit: `docs/workflow-batch-c-advance-supervise-migration-audit.md`. | Not complete; needs parity extraction before freeze. | `allowed_until_replaced`; current gate blocks by default but action remains. | Supervisor/control-loop compatibility, direct operator override with explicit legacy env. | Do not freeze/delete in batch. | Legacy task/dispatch reconciliation fails, or closeout executor parity absent. | Keep action implementation; rollback by disabling new parity path. |
| `workflow.checkpoint` | C | `legacy_active` | Recovery parity is not proven across v2 plan/node/worker/session state; Batch C substrate audit: `docs/workflow-batch-c-shared-substrate-audit.md`. | v2 checkpoint/readiness design still required. | `allowed_until_replaced`. | CLI/action registry, supervise closeout/recovery paths. | Do not freeze/delete. | Recovery evidence or incident closeout loses checkpoint boundary. | Keep existing checkpoint action. |
...
| `runtime.bridge.drain` | C | `legacy_active` / shared runtime bridge | Real v2 wrappers and adapter runner cutover are not fully deployed; bridge still drains queued runtime work; Batch C substrate audit: `docs/workflow-batch-c-shared-substrate-audit.md`. | `workflow.v2.adapter_runner.drain` only replaces v2 worker adapter job execution; generic `mixed_meeting_dispatches` drain replacement remains TBD. | `allowed_until_service_cutover`. | CLI runtime bridge, status guidance, control-loop drains, Hermers/OpenClaw adapter paths. | Do not freeze/delete. | Worker/dispatch queues stop producing terminal receipt evidence. | Revert runner migration; keep bridge drain action. |
...
| `route_shell.ingest`, `route_shell.route` | E | `live_clean` / `source_entrypoints_active` | Live dev-server state is clean: route-shell runtime rows are dormant/disabled/non-dispatchable, no queued route-shell dispatches, no route-shell message flows, and no active plugin hook observed. Source entrypoints still exist and can be activated by config/env or legacy callers. Batch E audit: `docs/workflow-batch-e-route-shell-removal-readiness-audit.md`. | `message_flow.send` and direct runtime adapter dispatch from `runtime_agents`; no v2 migration. | `must_not_depend`; v2 must not depend on route shell and must keep backend rejection. | CLI command, plugin action contract, optional Gateway before_dispatch hook, meeting.dispatch compatibility redirect, runtime.bridge.drain repair branch, archive regressions. | Do not migrate or delete; next batch must close source entrypoints and convert to retired/unknown-action behavior after regression update. | v2 worker backend accepts `openclaw_route_shell`, live queue contains route-shell work, plugin hook becomes active, or source-freeze removes needed message_flow/runtime dispatch evidence. | Revert source-freeze patch; keep archive compatibility until caller/hook/queue evidence is clean again. |
