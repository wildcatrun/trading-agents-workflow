...
| `route_shell.ingest`, `route_shell.route` | C | `deprecated` / `archive_no_migration` | Route shell is not an active executor for migrated Hermers agents; live migrated-agent route-shell rows are dormant/disabled, and old queued route-shell dispatches fail closed under registry validation. Batch C audit: `docs/workflow-batch-c-route-shell-archive-audit.md`. | `message_flow.send` and direct runtime adapter dispatch from `runtime_agents`. | `must_not_depend`; v2 must not depend on route shell. | CLI diagnostics, plugin action registry, migration telemetry, old queued dispatch fail-closed repair. | Do not migrate; do not use for active execution; remove only in a dedicated archive-removal batch after caller/queue/hook audit. | v2 worker backend accepts `openclaw_route_shell`, registry uses route shell as fallback executor, or old queued rows lose fail-closed evidence. | Keep current fail-closed route shell behavior until removal batch. |
