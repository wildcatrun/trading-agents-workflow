...
| `workflow.control_loop.tick`, `workflow.control_loop.job.*` | C | `shared_maintenance` / `legacy_supervise_lane_isolated` | Control-loop owns shared schedule, runtime drain, stale dispatch, message-flow, Human Gate, outbox, and repair lanes; only `workflow_supervise` is legacy lane. P23 adds an explicit disabled-lane mode proving shared maintenance can continue without claiming or seeding `workflow_supervise`. Batch C audit: `docs/workflow-batch-c-schedule-control-loop-audit.md`. | Shared maintenance service plus extracted/retired legacy supervise lane; v2 worker control-loop is only partial replacement. | `expected_shared_dependency`; legacy supervise `allowed_until_replaced`. | Status guidance, console recovery previews, control-loop tests. | Do not freeze/delete; keep default compatibility enabled until live legacy row/job audit and authorized cutover. | Queues stop reconciling scheduled dispatch/runtime drain/message-flow/Human Gate/outbox/dead-letter states, or disabled legacy lane still claims `workflow_supervise`. | Revert P23 isolation switch or re-enable default compatibility; keep action registry. |
