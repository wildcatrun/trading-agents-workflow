...
| `workflow.control_loop.tick`, `workflow.control_loop.job.*` | C | `shared_maintenance` / `legacy_supervise_lane_isolated` | Control-loop owns shared schedule, runtime drain, stale dispatch, message-flow, Human Gate, outbox, and repair lanes; only `workflow_supervise` is legacy lane. P23 adds an explicit disabled-lane mode, and P24 live audit found no `workflow_runs`, no `workflow_tasks`, no `workflow_supervise` jobs, and no state-log references on dev-server as of 2026-07-19. Batch C audit: `docs/workflow-batch-c-schedule-control-loop-audit.md`. | Shared maintenance service plus default-closed legacy supervise lane with opt-in compatibility override; v2 worker control-loop is only partial replacement. | `expected_shared_dependency`; legacy supervise remains `allowed_until_replaced` until the P25 cutover patch lands. | Status guidance, console recovery previews, control-loop tests; live audit artifact under `ops-artifacts/codex-working/20260719T-p24-live-legacy-supervise-audit`. | Do not freeze/delete the whole control loop; P25 may flip only the legacy supervise lane default if regression, server smoke, Gateway reload, and override path are preserved. | Queues stop reconciling scheduled dispatch/runtime drain/message-flow/Human Gate/outbox/dead-letter states, default tick still seeds `workflow_supervise`, or explicit opt-in compatibility breaks. | Revert P25 default flip or set `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_SUPERVISE_LANE=1`; keep action registry. |
