...
| `workflow.advance.preview`, `workflow.supervise.preview` | B | `read_surface_migrated` / `compat_shell_only` | P15/P16 moved semantic readiness/evidence-gap cards to v2 supervisor previews; P17 adds a v2 closeout preview for completed-plan closeout gaps. | `workflow.supervisor.readiness.preview`, `workflow.supervisor.next_actions.preview`, `workflow.supervisor.closeout.preview` | `must_not_depend`; v2 planning surfaces must not call legacy previews. | Explicit legacy diagnostics and historical task/run views. | Keep as manual compatibility diagnostics only; do not show in priority catalog or direct card buttons. | Console default v2 evidence-gap card reuses legacy preview, legacy preview returns to priority catalog, or v2 source references these actions. | Revert console/read-model migration or restore diagnostic references only. |
