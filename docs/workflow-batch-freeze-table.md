...
| `workflow.checkpoint` | C/P27 | `legacy_active` / `must_migrate_valid_parts` | P20 adds `workflow.supervisor.checkpoint` for v2 supervisor checkpoint boundaries, but P27 confirms parity is not complete: Human Gate archive closeout, legacy supervise escape hatch, context aliases, and operator CLI still depend on the legacy writer shape. | Partial: `workflow.supervisor.checkpoint.preview`, `workflow.supervisor.checkpoint` for v2 plan readiness decisions only. | `not_freeze_eligible_until_callers_retargeted`. | Human Gate archive closeout, legacy escape-hatch checkpointing, CLI/generic context recovery diagnostics. | Do not freeze/delete in P27; first build or retarget archive/shared checkpoint replacement. | Archive closeout loses checkpoint/resume boundary, explicit legacy recovery cannot write a checkpoint, or context aliases become accidental v2 calls without parity. | Keep legacy action; implement archive/shared checkpoint writer, retarget callers, then re-audit. |
