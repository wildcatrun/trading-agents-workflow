...
This table is a candidate/control table, not a deletion backlog and not a plan
to erase every v1-named surface. Batch freeze is not permission to delete
blindly. A candidate can move to actual runtime freeze/removal only when its row
matches one of two eligibility classes:

1. **Overlapped capability:** a v2 or shared replacement already owns the same
   useful production behavior, known callers are migrated or explicitly gated,
   and v2 no longer depends on the legacy entry point for normal flow.
2. **No-dependency retiree:** the block has no v2/runtime/live-state dependency,
   no active caller, and no remaining read/history/audit retention value beyond
   Git history.

Batch D/shared-substrate rows are protected infrastructure. They are not v1
retirement candidates unless a future architecture replaces the shared
capability itself and passes a separate architecture migration review.
...
| `workflow.task.draft`, `workflow.task.create.preview`, `workflow.task.launch.preview` | B | `compat_preview_only` / `compat_shell_only` | Drafting is pure preview compatibility, not v2 admission; metadata must not say `legacy_active`. | `workflow.v2.plan.preview`, template instantiate preview | `must_not_depend`; v2 must not rely on task draft. | CLI dry-run and operator diagnostics. | Keep explicit preview; do not expose as mutation/admission path. | It creates DB rows, appears as approved v2 plan creation, or metadata regresses to `legacy_active`. | Revert guard/docs; no state rollback expected for pure preview. |
| `workflow.task.list`, `workflow.tasks`, `workflow.task.launch.list` | B | `legacy_read_only_archive` / alias shells | Historical v1 rows/packages remain evidence, but should not define new work; metadata must not say `legacy_active`. | v2 read models for plans/nodes/workers/packages | `must_not_depend`; v2 read models may read shared DB tables but should not call legacy list actions. | CLI/MCP read-only diagnostics and archived evidence lookup. | Keep explicit list/read; hide from mutation-first docs. | New v2 cards or default planning buttons depend on legacy list actions, or metadata regresses to `legacy_active`. | Revert docs/registry change; no destructive migration. |
...
| `workflow.advance`, `workflow.supervise`, `workflow.supervisor` | C/P26 | `frozen_compatibility` | P24/P26 live audits found no dev-server legacy rows/tasks/jobs requiring standalone execution; P25 default-closed the automatic `workflow_supervise` lane. The handlers still mutate legacy tables, so they remain only behind strict escape hatch. | V2 readiness/next-actions/closeout/report plus shared dispatch reconcile/control-loop maintenance lanes; preview actions remain diagnostics. | `must_not_depend_by_default`; explicit `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_ACTIONS=1` is the short-term rollback path. | Historical diagnostics, emergency compatibility operation against proven legacy rows. | Keep default blocked; do not delete handler before final removal window closes. | Default API/CLI/Gateway path can execute legacy mutations without an explicit escape hatch, or the explicit compatibility path no longer reaches the old handler when needed. | Revert P26 metadata/test patch or set strict env escape hatch for one-off recovery; do not reopen control-loop seeding unless separately proven. |
| `workflow.checkpoint` | C/P34 | `frozen_writer_diagnostic` / `read_only_legacy_export` | P34 froze the legacy writer after v2/shared checkpoint and archive writers covered the valid write behavior; the remaining legacy value is explicit read-only export/diagnostic evidence. | `workflow.supervisor.checkpoint`, `workflow.archive.checkpoint`, `workflow.checkpoint.legacy_export` | `writer_must_not_depend`; normal v2/shared checkpoint writes must not call the legacy writer, while read-only legacy export remains allowed. | Human Gate archive closeout now targets shared archive checkpointing; operator diagnostics may call explicit read-only export; legacy escape hatch is not a normal v2 path. | Keep mutating legacy writer frozen; preserve read-only legacy export and explicit source-class routing. | Archive closeout/checkpoint preview fails, legacy export is unavailable, context aliases mutate through the legacy writer, or new v2 writes call `workflow.checkpoint`. | Revert P34 final checkpoint freeze, or route only explicit legacy source-class export for emergency diagnostics. |
| `workflow.pause`, `workflow.resume`, `workflow.stop`, `workflow.terminate` | C | `frozen_compatibility` | V2 intervention readiness, settlement evidence, and gated v2 plan-state transitions now cover the v1.0 hard-cutover path; the legacy execute shell only mutates v1 run state and remains too narrow for default use. | `workflow.v2.intervention_readiness.preview`, `workflow.v2.intervention_settlement.preview/record`, `workflow.v2.pause/resume/stop/terminate`. | `must_not_depend_by_default`; explicit `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_ACTIONS=1` is the short-term rollback path. | Emergency compatibility operation against proven legacy rows only. | Keep default blocked; do not delete handler before the compatibility window closes. | Default API/CLI/Gateway path can execute v1 lifecycle mutations without an explicit escape hatch, or v2 callers depend on legacy lifecycle writes. | Revert lifecycle freeze patch or set strict env escape hatch for one-off recovery. |
| `workflow.evaluate` | C | `frozen_compatibility` | V2 evaluator now owns snapshot checks and durable evaluator evidence through the shared verification rail. | `workflow.v2.evaluation_snapshot.preview` + `workflow.v2.evaluation.record` + `workflow.v2.validate`. | `allowed_until_replaced`; explicit evaluator escape hatch only. | Compatibility observation rows behind `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_EVALUATOR=1`. | Keep default blocked until removal window completes. | Legacy evaluator writes resume as a default caller path. | Use the explicit evaluator escape hatch for one release-window recovery only. |
...
| `workflow.control_loop.tick`, `workflow.control_loop.job.*` | C | `shared_maintenance` / `legacy_supervise_lane_default_closed` | Control-loop owns shared schedule, runtime drain, stale dispatch, message-flow, Human Gate, outbox, and repair lanes; only `workflow_supervise` is legacy lane. P23 isolated it, P24 found live state clean, and P25 flips the lane to default-closed with explicit opt-in compatibility. Batch C audit: `docs/workflow-batch-c-schedule-control-loop-audit.md`. | Shared maintenance service plus default-closed legacy supervise lane with opt-in compatibility override; v2 worker control-loop is only partial replacement. | `expected_shared_dependency`; legacy supervise is `default_closed_with_opt_in_compatibility`. | Status guidance, console recovery previews, control-loop tests; live audit artifact under `ops-artifacts/codex-working/20260719T-p24-live-legacy-supervise-audit`. | Do not freeze/delete the whole control loop; keep shared lanes active and keep legacy supervise opt-in until standalone legacy actions are archived. | Queues stop reconciling scheduled dispatch/runtime drain/message-flow/Human Gate/outbox/dead-letter states, default tick still seeds `workflow_supervise`, or explicit opt-in compatibility breaks. | Revert P25 default flip or set `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_SUPERVISE_LANE=1`; keep action registry. |
...
| `meeting.create`, `meeting.append`, `meeting.command`, `meeting.summary`, `meeting.close`, `meeting.handoff`, `meeting.artifact`, `meeting.state`, `meeting.action_item`, `meeting.decision`, `meeting.minutes`, `meeting.notify`, `meeting.index`, `cat_claw.observe/minutes/digest/notify` | C/P62 | `frozen_compatibility` | V2 already has the same class of multi-agent discussion/evidence capability through plan nodes, manager reviews, owner reviews, task-group package, Cat Brain audit, Cat Claw audit, info stack, notification preview, and Human Gate/outbox. No extra v2 feature is required to retire these v1 meeting-room write surfaces. | `workflow.v2.plan.create`, `workflow.v2.info_stack.record`, `workflow.v2.manager_review.record`, `workflow.v2.owner_review.record`, `workflow.v2.task_group_package.record`, `workflow.v2.cat_brain_audit.record`, `workflow.v2.cat_claw_audit.record`, `workflow.v2.notification.preview`, shared Human Gate/outbox. | `must_not_depend_by_default`; explicit `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_ACTIONS=1` is the short-term archive compatibility path. | Historical meeting files, archive diagnostics, and emergency compatibility writes only. `meeting.show/list/validate` remain archive/read diagnostics. | Keep default blocked and hidden from default full-tool enum; do not add v2 features just to imitate v1 meeting rooms. | Any default API/CLI/Gateway path can create/append/close/write new v1 meeting discussion state without explicit escape hatch, or v2 task-group flow starts depending on v1 meeting files. | Revert P62 freeze patch or set strict env escape hatch for one-off archive recovery; do not reopen as normal workflow surface. |
| `meeting.dispatch` | C/P63 | `compatibility_writer_observation` | `dispatch.package.preview/create` is the canonical dispatch package surface, but `dispatch.package.create` still delegates to the registered compatibility writer and shared `mixed_meeting_dispatches` ledger. P63 live audit found no active queued/sent rows and no workflow operation rows for either public action. | `dispatch.package.preview` + `dispatch.package.create`. | `allowed_until_replaced`; new callers should use canonical create, while public compatibility use remains observable. | CLI, OpenClaw plugin actions, dispatch/receipt evidence paths. | Do not freeze/delete until observation window proves no public compatibility caller and canonical create no longer delegates to `meeting.dispatch`. | Runtime dispatch or receipt evidence disappears, or public callers lose the compatibility shell before observation closes. | Keep dispatch module and registry; revert metadata split only if tooling requires the old legacy-active bucket. |
| `meeting.ingest`, `meeting.resume`, `meeting.disperse`, `meeting.runtime_participant` | C | `legacy_active` / shared adapter | These are not only meetings; they still provide receipt/runtime adapter evidence or compatibility fan-out surfaces and need a separate adapter parity audit. Batch C substrate audit: `docs/workflow-batch-c-shared-substrate-audit.md`. | shared runtime adapter evidence plus archive/maintenance surfaces. | `allowed_until_replaced`. | CLI, OpenClaw plugin actions, receipt/runtime evidence paths. | Do not freeze/delete without adapter parity audit, call-site migration, and observation window. | Runtime receipt, resume, fan-out, or participant evidence disappears. | Keep meeting/runtime adapter module and registry. |
