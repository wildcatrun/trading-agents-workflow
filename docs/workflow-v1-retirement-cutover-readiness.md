...
| `workflow.task.draft` | `compat_preview_only` authoring shell | `workflow.v2.plan.preview/create` and template instantiate | Keep explicit preview-only compatibility; do not present as v2 admission or active task creation. |
| `workflow.task.list`, `workflow.tasks`, `workflow.task.launch.list` | `legacy_read_only_archive` / alias shells | v2 plan/package read models and explicit archive reads | Keep explicit archive diagnostics/history until final removal window; v2 planning must not depend on these names. |
| `workflow.pause`, `workflow.resume`, `workflow.stop`, `workflow.terminate` | `frozen_compatibility` | `workflow.v2.intervention_readiness.preview`, `workflow.v2.intervention_settlement.preview`, and `workflow.v2.pause/resume/stop/terminate` plan-state transitions | Default blocked; the v1.0 hard-cutover decision treats read-only settlement evidence plus gated v2 transitions as sufficient for new callers. Use `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_ACTIONS=1` only as a short-term compatibility escape hatch. |
| `workflow.evaluate` | `frozen_compatibility` | `workflow.v2.evaluation_snapshot.preview`, `workflow.v2.evaluation.record`, `workflow.v2.validate` | Default blocked; durable evaluator evidence belongs on the v2 snapshot/record path. Use `TRADING_AGENTS_WORKFLOW_ENABLE_LEGACY_EVALUATOR=1` only during the observation window. |
