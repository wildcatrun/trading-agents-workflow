...
Cat Claw should provide the approved alternatives as `buttons`, `options`, `alternatives`, or `plans`. The plugin stores each button in `human_gate_buttons`, renders the same choices in the Human Gate console, sends Telegram inline buttons through OpenClaw `presentation`, and handles the `tawhg:<token>` callback directly. Telegram button coloring must use the Bot API `style` field (`primary`, `success`, or `danger`) so the entire button is styled; color-square emoji in labels are not an acceptable substitute. Fixed Human Gate styles are: plan A/B/C/D option buttons use `success`; reject/return buttons use `danger`; pause workflow uses `primary`; terminate workflow uses `danger`. Telegram has no separate yellow style in this three-style surface, so pause uses `primary`. The plugin appends control buttons for "退回补证/修改", "暂停工作流", and "终止工作流". "终止工作流" means Flashcat considers the work complete and reviewed, so the workflow is archived with a checkpoint and closeout dispatches to `main` and `cat_claw`; it remains resumable later by workflow id/checkpoint.

Human Gate button callbacks are currently two-step until Telegram Web App review forms are available. The first Telegram button click locks the selected option and changes the button row to `feedback_pending`; it does not complete the Human Gate. Flashcat must then submit original words or review feedback with the exact callback token from the button prompt, for example `/hgate tawhg:<token> 这里写闪电猫原话或审核意见`, or pass the same token and text through `human_gate.feedback`. Bare `/hgate <text>` is rejected because it cannot be safely matched when multiple Human Gate requests are pending. Only after token-bound feedback is sent does the workflow record the final Human Gate status, save `flashcatOriginalWords` in the button row, Human Gate record, workflow payload, meeting resume payload, and next dispatch payload, and resume/close the workflow from the selected boundary. The selected button status remains the formal decision, while Flashcat's original words are binding guidance for the next workflow direction, scope, and boundaries. The target design is a Telegram `web_app` button per Human Gate option: the button opens a token-bound review form, and the textarea submit completes the same Human Gate decision object in one user interaction.
