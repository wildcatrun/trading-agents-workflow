...
Human Gate button callbacks are two-step. The first Telegram button click locks the selected option and changes the button row to `feedback_pending`; it does not complete the Human Gate. Flashcat must then submit original words or review feedback, for example `/hgate 这里写闪电猫原话或审核意见`, or pass the same text through `human_gate.feedback`. Only after that feedback is sent does the workflow record the final Human Gate status, save `flashcatOriginalWords` in the button row, Human Gate record, workflow payload, meeting resume payload, and next dispatch payload, and resume/close the workflow from the selected boundary. The selected button status remains the formal decision, while Flashcat's original words are binding guidance for the next workflow direction, scope, and boundaries.
