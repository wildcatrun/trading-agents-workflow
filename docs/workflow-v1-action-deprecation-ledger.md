...
| `workflow.advance`, `workflow.supervise` | `legacy_active` | semantic supervisor readiness / next-actions plus v2 executor parity | P19 covers completed-plan Cat Claw closeout, P20 adds v2 checkpoint writer, P21 removes direct supervisor runtime drain, and P22 adds blocked/Human Gate pending report parity. Remaining blockers are legacy dispatch sync, task/run transition, and active legacy row retirement/extraction. See `docs/workflow-p14-advance-supervise-freeze-readiness-audit.md`. |
