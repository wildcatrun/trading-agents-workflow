...
| `workflow.advance`, `workflow.supervise` | `legacy_active` | semantic supervisor readiness / next-actions plus v2 executor parity | P14 refresh confirms P12/P13 cover read-only readiness and next-action planning, but mutating dispatch sync, task transition, checkpoint, runtime-drain, and Cat Claw closeout executor behavior are not freeze-ready. See `docs/workflow-p14-advance-supervise-freeze-readiness-audit.md`. |
