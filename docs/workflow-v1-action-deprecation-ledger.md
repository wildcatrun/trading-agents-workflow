...
| `workflow.task.list`, `workflow.tasks` | `legacy_read_only_archive` / alias shell | read/history compatibility | Keep only for archived v1 task history lookup until final removal evidence exists. |
...
| `meeting.dispatch` | `compatibility_writer_observation` | `dispatch.package.preview/create` | New callers should use `dispatch.package.create`; keep `meeting.dispatch` as the registered compatibility writer until the observation/removal window closes. See `docs/workflow-p63-meeting-dispatch-observation-audit.md`. |
| `meeting.ingest`, `meeting.resume`, `meeting.disperse` | `legacy_active` / shared adapter | shared runtime adapter evidence and archive/maintenance surfaces | Preserve current OpenClaw/Hermers delivery behavior until explicit replacement. |
