...
| `workflow.advance`, `workflow.supervise` | `legacy_active` | semantic supervisor readiness / next-actions plus v2 executor parity | P19 covers completed-plan Cat Claw closeout, P20 adds v2 checkpoint writer, P21 removes direct supervisor runtime drain, P22 adds blocked/Human Gate pending report parity, P23 isolates the control-loop `workflow_supervise` lane, P24 live audit found no dev-server legacy rows/jobs/log refs requiring that lane, and P25 makes the lane default-closed with opt-in compatibility. Remaining blocker is any future decision on archiving the standalone `workflow.advance` / `workflow.supervise` actions. See `docs/workflow-p14-advance-supervise-freeze-readiness-audit.md`, `docs/workflow-p23-control-loop-legacy-supervise-lane-isolation.md`, `docs/workflow-p24-live-legacy-supervise-audit.md`, and `docs/workflow-p25-default-close-legacy-supervise-lane.md`. |
