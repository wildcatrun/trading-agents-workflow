...
| `workflow.advance`, `workflow.supervise` | `legacy_active` | semantic supervisor readiness / next-actions plus v2 executor parity | P19 covers completed-plan Cat Claw closeout, P20 adds v2 checkpoint writer, P21 removes direct supervisor runtime drain, P22 adds blocked/Human Gate pending report parity, and P23 isolates the control-loop `workflow_supervise` lane behind an explicit compatibility switch. Remaining blockers are legacy dispatch sync, task/run transition, live legacy row/job audit, and default cutover authorization. See `docs/workflow-p14-advance-supervise-freeze-readiness-audit.md` and `docs/workflow-p23-control-loop-legacy-supervise-lane-isolation.md`. |
