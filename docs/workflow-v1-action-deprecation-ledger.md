...
| `workflow.advance`, `workflow.supervise` | `legacy_active` | semantic supervisor readiness / next-actions plus v2 executor parity | P19 covers completed-plan Cat Claw closeout, P20 adds v2 checkpoint writer, P21 removes direct supervisor runtime drain, P22 adds blocked/Human Gate pending report parity, P23 isolates the control-loop `workflow_supervise` lane behind an explicit compatibility switch, and P24 live audit found no dev-server legacy rows/jobs/log refs requiring that lane. Remaining blockers are the P25 default-close cutover patch, explicit opt-in compatibility, and any future decision on archiving the standalone `workflow.advance` / `workflow.supervise` actions. See `docs/workflow-p14-advance-supervise-freeze-readiness-audit.md`, `docs/workflow-p23-control-loop-legacy-supervise-lane-isolation.md`, and `docs/workflow-p24-live-legacy-supervise-audit.md`. |
