...
- extract the legacy `workflow_supervise` lane from the shared control loop before freezing any legacy orchestration code;
...
| Legacy workflow supervise lane | Control loop seeds and executes `workflow_supervise` jobs for legacy `workflow_runs`, with idle cooldowns and runtime drains after dispatch. | Extract or retire after v2 supervisor/readiness and closeout parity is proven. | `legacy_lane_to_split`. | Freezing the whole control loop to retire this lane also freezes shared maintenance. |
...
| `workflow_supervise` lane inside control loop | Split before retirement. | This is the legacy orchestration lane; it should not be used as a reason to delete the shared maintenance executor. |
...
2. Extract the `workflow_supervise` seed and execution lane from generic `workflowControlLoopTick` into an explicitly named compatibility module or retire it after v2 parity is proven.
...
6. Add regression that proves disabling `workflow_supervise` compatibility does not stop scheduled dispatch, runtime drain, stale dispatch reconcile, message-flow reconcile, Human Gate, outbox, or job requeue repair.
