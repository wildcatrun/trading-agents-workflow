...
| Cat Claw closeout/report dispatch | `workflow.supervise` dispatches `workflow_secretary_report` / `human_gate_report` to `openclaw:cat_claw` for `cat_claw_summary_required`, `blocked`, or `human_gate_pending`. | P19 adds completed-plan `workflow.supervisor.closeout`; P22 adds checkpoint-gated `workflow.supervisor.report` for blocked and Human Gate pending v2 plans. | `v2_parity_available_for_v2_rows`; legacy row retirement still pending. | V2 plans have report parity, but legacy workflows still need archive/retirement or legacy sync extraction before mutating supervise can freeze. |
...
5. Replace Cat Claw closeout/report dispatch with v2 executors: P19 completed-plan closeout and P22 blocked/Human Gate pending report parity are available; remaining work is proving legacy row retirement or extracting legacy sync.
