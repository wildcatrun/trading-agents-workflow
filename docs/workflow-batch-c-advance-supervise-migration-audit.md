...
| Cat Claw closeout/report dispatch | `workflow.supervise` dispatches `workflow_secretary_report` / `human_gate_report` to `openclaw:cat_claw` for `cat_claw_summary_required`, `blocked`, or `human_gate_pending`. | `workflow.supervisor.next_actions.preview` explicitly marks v2 closeout as `replacement_gap`; v2 Human Gate package/request paths do not yet replace final closeout report execution. | `must_migrate`. | Completed/blocked/human-gate legacy workflows lose formal Cat Claw closeout/report path. |
