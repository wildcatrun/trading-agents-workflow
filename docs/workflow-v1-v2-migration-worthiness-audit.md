...
| `workflow.checkpoint` | Legacy checkpoint artifact path and recovery evidence. | V2/shared writers now cover supervisor checkpoint and archive checkpoint; legacy row-shape inspection is read-only through `workflow.checkpoint.legacy_export`. | `compat_shell_only` | P34 freezes the mutating legacy writer as a diagnostic. Keep the action name only to guide callers to explicit v2/shared writers or read-only legacy export. |
...
| `workflow.advance`, `workflow.supervise` | Legacy mechanical progression and supervisor diagnostics. | Supervisor readiness/next-actions/checkpoint/report/closeout plus shared dispatch reconcile and maintenance lanes. | `compat_shell_only` | P19-P26 completed the freeze path: standalone mutating entry points are default-disabled compatibility executors, and the `workflow_supervise` lane is default-closed. Do not use for new v2 planning. |
...
| `workflow.schedule.*` | Schedule persistence and raw schedule dispatch control. | Approved-template / Human-Gate-approved v2 plan schedule binding already guards production admission; raw schedule dispatch remains diagnostics-only. | `shared_scheduler_keep` | Keep as governed scheduler substrate. Do not build a parallel v2 scheduler; audit aliases later. |
| `workflow.control_loop.tick`, `workflow.control_loop.job.*` | Shared mechanical maintenance, retry, dead-letter, runtime drain assistance, and legacy supervise compatibility lane. | V2 control-loop owns worker-run lifecycle only; generic control loop still owns schedules, stale dispatch, message-flow, Human Gate, outbox, and repair. | `shared_maintenance_keep` with legacy lane default-closed | Keep shared maintenance. Only the `workflow_supervise` lane is legacy and already default-closed. |
...
| `runtime.bridge.drain` | Shared runtime bridge drain for queued work and Hermers/OpenClaw adapter paths. | V2 adapter job/runner contract covers v2 worker jobs, but shared dispatch drain still needs the existing bridge. | `shared_runtime_keep` | Preserve bridge evidence semantics; do not route `openclaw_route_shell`. |
...
| `route_shell.*` | Historical route-shell ingress/routing. | Runtime registry + message_flow/runtime adapters. | `source_deleted` | Batch F/G closed public entry points and deleted archived implementation; retain only historical vocabulary/read evidence and retired-runtime rejection behavior. |
...
| `workflow.evaluate` and aliases | Active orchestration-level evaluator over tasks, dispatches, runtime runs, Human Gates, side effects, incidents, artifacts, and verification results. | `workflow.v2.evaluation_snapshot.preview` and `workflow.v2.validate` cover part of the replacement surface. | `legacy_active` until evaluator parity | Extract remaining evidence persistence and blocker checks into v2/shared validators before retiring the old evaluator action. |
...
| `intervention.*` pause/resume/stop/rerun previews and actions | Operational control surface. | Shared policy/Human Gate/incident rails, v2 intervention readiness, and v2 plan-state transition actions. | `legacy_active` until full v2 intervention parity | Keep hard gates; v2 plan-state transitions exist, but legacy lifecycle freeze still needs active worker/adapter/session/dispatch/outbox/Human Gate/side-effect/incident settlement parity. |
...
- `route_shell.*`: source-deleted; keep only historical vocabulary/read evidence
  and retired-runtime rejection behavior.
